...
@section('m_wh_location','active')
@section('firstmenu','Master Data')
@section('menu','Material Group')
@section('sub_menu','Warehouse Location')
...
<p></p>
						<div class="form">
						  <div class="form-group">
						   <div class="col-md-10">
              <input type="text" class="form-control" id="warehouse_search" placeholder="Enter WH Location" onkeypress="search_enter(event)">
						  </div>
            </div>
...
function list_data()
{
   var page           = 1;
   var pagination     = '';
   var warehouse_loc  = $("#warehouse_search").val();
  $.ajax({
          type: "POST",
           url: 'whLoc/list_data',
           data: {
    ...
    else
    {
      ...
      $.ajax({
          ...
          success: function(data)
          {
            ...
            {
                if (data.errors.warehouse_location)
                {
                      alertMsg(data.errors.warehouse_location,'error');
                }
          ...
          error: function()
          {
                    swal("Info System", 'Sorry, there is a problem with the system', "error");
          }
      });
  });
